...
function App() {
    ...
    const presentClients = presenceData.map((msg, index) => <li key={index}>{msg.clientId}: {msg.data}</li>);
    ...
    return (
        <div className="App">
            ...
            <div>
                <button onClick={() => { channel.publish("test-message", { text: "message text" }); }}>Send Message</button>
                <button onClick={() => { updateStatus("hello"); }}>Update status to hello</button>
            </div>
