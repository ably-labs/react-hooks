...
describe('useChannel', () => {
    ...
    it('can register a connection state listener for all state changes', async () => {
        renderInCtxProvider(
            ablyClient,
            <UseConnectionStateListenerComponent></UseConnectionStateListenerComponent>
        );
        ...
        expect(screen.getAllByRole('connectionState')[0].innerHTML).toEqual(
            'connected'
        );
    });
...
const UseConnectionStateListenerComponent = () => {
    const [connectionState, setConnectionState] =
        useState<Types.ConnectionState>('initialized');
